...
import { MessageSquare, Trash2, User, AtSign, Calendar, ArrowUpRight } from "lucide-react";
...
import AdminCommentReply from "@/features/admin/components/AdminCommentReply";
...
export default async function AdminCommentsPage({ searchParams }: AdminCommentsPageProps) {
    ...
    const { comments, totalPages } = await getAllComments({ page: currentPage, limit: 10 });
    ...
    return (
        <div className="space-y-10">
            ...
            {totalPages > 1 && (
                <Pagination currentPage={currentPage} totalPages={totalPages} baseUrl="/admin/comments" />
            )}
        </div>
    );
}
