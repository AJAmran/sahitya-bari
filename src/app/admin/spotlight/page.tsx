import { Plus, Edit3, Trash2, Star, Calendar, CheckCircle2, XCircle, Image as ImageIcon } from 'lucide-react';
import Link from 'next/link';
import { deleteSpotlight } from "@/features/spotlight/actions";
import { getAllSpotlightsFresh } from "@/features/spotlight/api";
...
export default async function AdminSpotlightPage({ searchParams }: AdminSpotlightPageProps) {
  const params = await searchParams;
  const currentPage = Number(params.page) || 1;
  const { spotlights, totalPages } = await getAllSpotlightsFresh({ page: currentPage, limit: 10 });
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[var(--glass-border)]/50">
              {spotlights.map((spotlight: any) => (
                <tr key={spotlight._id} className="group hover:bg-[var(--primary)]/[0.02] transition-colors">
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex items-center justify-end gap-2 transition-all duration-300">
                      <Link
                        href={`/admin/spotlight/${spotlight._id}/edit`}
                      ...
                      <DeleteForm action={deleteSpotlight.bind(null, spotlight._id)}>
                        <button
                          type="submit"
