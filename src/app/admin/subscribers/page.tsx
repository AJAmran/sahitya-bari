...
import { Trash2, UserPlus, Mail, Calendar, CheckCircle2, XCircle } from "lucide-react"
import DeleteForm from "@/features/admin/components/DeleteForm"
import { deleteSubscriber } from "@/features/newsletter/actions";
import { getSubscribersFresh } from "@/features/newsletter/api";

export default async function AdminSubscribersPage() {
    const subscribers = await getSubscribersFresh();
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-[var(--glass-border)]/50">
                            {subscribers.map((subscriber: any) => (
                                <tr key={subscriber._id} className="group hover:bg-[var(--primary)]/[0.02] transition-colors">
                                    ...
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex items-center justify-end gap-2 transition-all duration-300">
                                            <DeleteForm action={deleteSubscriber.bind(null, subscriber._id)}>
                                                <button
                                                    type="submit"
