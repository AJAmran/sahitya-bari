...
import { submitComment, submitReply, deleteComment, toggleCommentApproval } from "../actions"
    ...
    return (
        <div className="mt-12 pt-10 border-t border-[var(--glass-border)]">
            <div className="space-y-8">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-[var(--glass-border)] pb-6">
                    <div className="space-y-1">
                        <h2 className="text-xl font-black font-bengali text-[var(--foreground)]">সংবাদ বিনিময়</h2>
                        <p className="text-[0.65rem] font-bold text-[var(--foreground)]/45 leading-relaxed tracking-wider uppercase">
                            Join the dialogue. Share your perspectives.
                        </p>
                    </div>
                    <div className="px-3 py-1.5 rounded-full bg-[var(--surface-50)] border border-[var(--glass-border)] text-[0.6rem] font-black uppercase tracking-widest text-[var(--primary)] shadow-[var(--shadow-sm)]">
                ...
                <div className="grid grid-cols-1 gap-8 items-start">
                    ...
                    <div className="lg:col-span-5 lg:sticky lg:top-24">
                        <div className="glass-card p-5 rounded-[1.5rem] space-y-5 shadow-[var(--shadow-sm)]">
                            <h3 className="text-[0.65rem] font-black uppercase tracking-[0.3em] text-[var(--foreground)]/30">New Entry</h3>

                            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <label className="text-[0.6rem] font-black uppercase tracking-widest text-[var(--foreground)]/45 pl-1">Name</label>
                                        <div className="relative">
                                            <User size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--foreground)]/30" />
                                            <input
                                                {...register("author")}
                                                placeholder="Your Name"
                                                className="w-full bg-[var(--surface-200)] rounded-md border border-[var(--glass-border)] pl-9 pr-3 h-10 text-xs font-medium text-[var(--foreground)] focus:ring-2 focus:ring-[var(--primary)]/20 outline-none transition-all placeholder:text-[var(--foreground)]/30"
                                            />
                                        </div>
                                        {errors.author && <p className="text-[var(--destructive)] text-[0.6rem] font-bold uppercase pl-1">{String(errors.author.message)}</p>}
                                    </div>

                                    <div className="space-y-1.5">
                                        <label className="text-[0.6rem] font-black uppercase tracking-widest text-[var(--foreground)]/45 pl-1">Email</label>
                                        <div className="relative">
                                            <AtSign size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--foreground)]/30" />
                                            <input
                                                {...register("email")}
                                        ...
                                        {errors.email && <p className="text-[var(--destructive)] text-[0.6rem] font-bold uppercase pl-1">{String(errors.email.message)}</p>}
                                    </div>
                                </div>

                                <div className="space-y-1.5">
                                    <label className="text-[0.6rem] font-black uppercase tracking-widest text-[var(--foreground)]/45 pl-1">Comment</label>
                                    <textarea
                                        {...register("content")}
                                        placeholder="Type your thoughts..."
                                        className="w-full bg-[var(--surface-200)] rounded-md border border-[var(--glass-border)] p-4 text-xs font-medium text-[var(--foreground)] focus:ring-2 focus:ring-[var(--primary)]/20 outline-none h-28 transition-all resize-none placeholder:text-[var(--foreground)]/30"
                                    />
                                    {errors.content && <p className="text-[var(--destructive)] text-[0.6rem] font-bold uppercase pl-1">{String(errors.content.message)}</p>}
                                </div>

                                <button
                                    type="submit"
                                    disabled={submitting}
                                    className="w-full h-10 rounded-[var(--radius-lg)] bg-[var(--foreground)] text-[var(--background)] font-black text-[0.65rem] uppercase tracking-[0.25em] flex items-center justify-center gap-2 active:scale-95 transition-all shadow-[var(--shadow-sm)]"
                                >
