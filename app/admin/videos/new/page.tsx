...
export default function NewVideoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);
    const toastId = toast.loading('Synchronizing video data...');

    try {
      const formData = new FormData(e.currentTarget);
      await createVideo(formData);
      toast.success('Video cataloged successfully!', { id: toastId, icon: <CheckCircle2 className="text-green-500" /> });
      router.push('/admin/videos');
      router.refresh();
    } catch (error) {
      console.error('Failed to create video:', error);
      toast.error('Failed to add video. Please verify the ID.', { id: toastId });
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="flex flex-col gap-3">
                <label className="flex items-center gap-3 cursor-pointer group/toggle">
                  <div className="relative">
                    <input type="checkbox" name="isUpcoming" className="sr-only peer" />
                ...
                <label className="flex items-center gap-3 cursor-pointer group/toggle">
                  <div className="relative">
                    <input type="checkbox" name="isPopular" className="sr-only peer" />
