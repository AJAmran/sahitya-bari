...
import { Trash2, Mail, MailOpen, CheckCircle, Clock, User, MessageSquare, AtSign, Calendar, Sparkles } from "lucide-react"
...
export default async function AdminMessagesPage() {
    await dbConnect()
    const rawMessages = await ContactMessage.find()
        .sort({ createdAt: -1 })
        .lean()
        ...
        <div className="space-y-10">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="px-6 py-3 rounded-[var(--radius-md)] bg-[var(--info)]/10 border border-[var(--info)]/20 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xl font-black text-[var(--foreground)] leading-none">{messages.length}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-6">
                {messages.map((message: any) => (
                    <div
                        key={message.id}
                        className={`group relative bg-[var(--surface-100)]/40 backdrop-blur-xl rounded-[var(--radius-xl)] border transition-all duration-500 hover:scale-[1.01] hover:shadow-2xl ${message.isRead
                        ...
                        <div className="p-8">
                            <div className="flex flex-col lg:flex-row gap-8">
                                ...
                                <div className="flex lg:flex-col items-center justify-end gap-3 shrink-0">
                                    <form action={toggleRead.bind(null, message.id, message.isRead)}>
                                        <button
                                            type="submit"
                                    ...
                                    <DeleteForm action={deleteMessage.bind(null, message.id)}>
                                        <button
                                            type="submit"
